...
struct delayed_work  mac_get_work;
struct workqueue_struct *mac_get_wq;

static void mac_get_fuc(struct work_struct *pwork) //(unsigned long _data)
{
    struct  file *file = NULL;
    file = filp_open(WLAN_MAC_FILE, O_RDONLY,0);
    if(IS_ERR(file)) {
        printk("waiting for get mac.\n", WLAN_MAC_FILE);
        queue_delayed_work(mac_get_wq, &mac_get_work, msecs_to_jiffies(2000));
    } else {
        platform_driver_register(&rk29_vmac_driver);
    }
}

static int __init vmac_init(void)
{
    INIT_DELAYED_WORK(&mac_get_work, mac_get_fuc);
    mac_get_wq = create_workqueue("mac_get_wkqueue");
    if (mac_get_wq){
        queue_delayed_work(mac_get_wq, &mac_get_work, msecs_to_jiffies(10000));
    }
}
