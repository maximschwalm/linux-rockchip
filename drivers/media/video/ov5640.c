...
*/
	static int sensor_parameter_record(struct i2c_client *client)
	{
     ...
     ulCapture_Exposure = ulCapture_Exposure*2/3; 
...
static int sensor_activate_cb(struct i2c_client *client)
{
	SENSOR_DG("%s",__FUNCTION__);
	  struct regulator *ldo_focus = NULL;
  	ldo_focus = regulator_get(NULL, "afvdd_pmu");
	  if (ldo_focus == NULL || IS_ERR(ldo_focus)){
		  printk("get focus ldo failed!\n");
		}
		else{
		  if(regulator_is_enabled(ldo_focus)<=0)
		  {	
        regulator_set_voltage(ldo_focus, 2800000, 2800000);
		    if (!regulator_enable(ldo_focus))
		      regulator_put(ldo_focus);
	    }
    }
   	mdelay(10);//wait regulater operation finished
	return 0;
}
/*
...
static int sensor_deactivate_cb(struct i2c_client *client)
{
	...
	if (sensor->info_priv.funmodule_state & SENSOR_INIT_IS_OK) {
	  ...
	  if (ldo_focus == NULL || IS_ERR(ldo_focus)){
		  ...
		  if(regulator_is_enabled(ldo_focus)>0)
		  {	
		    if (!regulator_disable(ldo_focus))
		      regulator_put(ldo_focus);
	    }
    }
      mdelay(10);//wait regulater operation finished
