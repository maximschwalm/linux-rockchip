...
extern void usb20host_power_enable(int enable);
#endif
static struct wake_lock axp_wake_lock;
static int status_lock=0;
int pmu_usbvolnew = 0;
int pmu_usbcurnew = 0;
...
extern int dwc_connect_status(void) ;
...
static void axp_charger_update_state(struct axp_charger *charger)
{
  ...
  charger->charge_on = ((val[0] >> 7) & 0x01);
	if( 1 == charger->ext_valid){  // charge
		if(0 == status_lock && charger->rest_vol!=100){			
			wake_lock(&axp_wake_lock);  //lock
			printk("wake_lock +++\n");
			status_lock = 1; 
		}
		if(1 == status_lock  && charger->rest_vol==100){			
			wake_unlock(&axp_wake_lock);  //unlock
			printk("wake_lock ---\n");
			status_lock = 0; 
		}
		...
		//gpio_set_value(RK30_PIN0_PA1, GPIO_LOW); //connect VBUS to AC
	}
}
...
static void axp_change(struct axp_charger *charger)
{
  ...
  printk("charger->usb_valid = %d\n",charger->usb_valid);
	if(!charger->usb_valid){
        ...
        // fky_debug : add
        #if  1
        if(4 == dwc_connect_status())	// ac charge
        {
            // axp_usb_curlim(0);
            axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x03);
            printk("fky_debug : axp22-sply.c : axp_change() : i_vbus no limited.\r\n") ;
        }
        else
        {
            ...
            axp_clr_bits(charger->master, AXP22_CHARGE_VBUS, 0x02);
            axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x01);
            printk("fky_debug : axp22-sply.c : axp_change() : i_vbus <= 0.5A.\r\n") ;
        }
        #endif
...
{
#if  0
    printk("fky_debug : axp22-sply.c : axp_pressshort() : start.\r\n") ;
	DBG_PSY_MSG("press short\n");
 	...
 	input_report_key(powerkeydev, KEY_POWER, 0);
 	input_sync(powerkeydev);
    printk("fky_debug : axp22-sply.c : axp_pressshort() : end.\r\n") ;
#endif
}
...
{
#if  0
    printk("fky_debug : axp22-sply.c : axp_keyup() : start.\r\n") ;
	DBG_PSY_MSG("power key up\n");
	input_report_key(powerkeydev, KEY_POWER, 0);
	input_sync(powerkeydev);
    printk("fky_debug : axp22-sply.c : axp_keyup() : end.\r\n") ;
#endif
}
...
{
#if  0
    printk("fky_debug : axp22-sply.c : axp_keydown() : start.\r\n") ;
	DBG_PSY_MSG("power key down\n");
	input_report_key(powerkeydev, KEY_POWER, 1);
	input_sync(powerkeydev);
    printk("fky_debug : axp22-sply.c : axp_keydown() : end.\r\n") ;
#endif
}
...
static void axp_capchange(struct axp_charger *charger)
{
    ...
    if((charger->bat_det == 0) || (charger->rest_vol == 127)){
  	charger->rest_vol = 100;
		...
		//gpio_set_value(RK30_PIN0_PA1, GPIO_HIGH); //disconnect VBUS to AC
  }
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(axp_debug){
		...
		axp_read(charger->master,0x00,temp_val);
		DBG_PSY_MSG("Axp22 REG_00 = %x\n",temp_val[0]);
		axp_read(charger->master,0x01,temp_val);
		DBG_PSY_MSG("Axp22 REG_01 = %x\n",temp_val[0]);
		axp_read(charger->master,0x33,temp_val);
		DBG_PSY_MSG("Axp22 REG_33 = %x\n",temp_val[0]);
...
// fky_debug : add
void  set_usb_current_500ma(void)
{
    struct axp_charger * charger ;
    charger = axp_charger;
    axp_clr_bits(charger->master, AXP22_CHARGE_VBUS, 0x02);
...
static void axp_usb(struct work_struct *work)
{
	...
	printk("fky_debug : axp22-sply.c : axp_usb() : axp_debug=%d, axp_usbcurflag=%d.\r\n",axp_debug,axp_usbcurflag) ;
	if(axp_debug)
	{
...
//        axp_usb_curlim(0);
		axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x03);
#if(VERSION_TYPE > VERSION_TYPE_0)
        printk("fky_debug : axp22-sply.c : axp_usb() : i_vbus no limited.\r\n") ;
#endif
    }
    else
    {
//        axp_usb_curlim(USBCURLIM);
		...
		axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x01);
#if(VERSION_TYPE > VERSION_TYPE_0)
        printk("fky_debug : axp22-sply.c : axp_usb() : i_vbus <= 0.5A.\r\n") ;
#endif
    }
...
static int axp_battery_probe(struct platform_device *pdev)
{
  ...
  if (!powerkeydev) {
	...
	status_lock = 0;
	wake_lock_init(&axp_wake_lock, WAKE_LOCK_SUSPEND, "axp228_lock");	
...
//  printk("fky_debug : axp22-sply.c : axp_battery_probe() : chgcur = %d\r\n", pdata->chgcur) ;
//  printk("fky_debug : axp22-sply.c : axp_battery_probe() : chgvol = %d\r\n", pdata->chgvol) ;
...
//  printk("add axp_battery_event to notifier[%2x]\n", axp_battery_event);
  ...
  if((USBCURLIM) && (USBCURLIMEN)){
	...
	axp_clr_bits(charger->master, AXP22_CHARGE_VBUS, 0x02);
	axp_set_bits(charger->master, AXP22_CHARGE_VBUS, 0x01);
	printk("fky_debug : axp22-sply.c : axp_battery_probe() : i_vbus <= 0.5A.\r\n") ;
  ...
  axp_read(charger->master,0xd0,&tqp);//tqp test
  printk("tqp---adrr2=%d\n",tqp);
	/* pok open time set */
...
//probe 时初始化RDC，使其提前计算正确的OCV，然后在此处启动计量系统
  ...
  axp_clr_bits(charger->master,AXP22_COULOMB_CTL, 0x40); //disable
