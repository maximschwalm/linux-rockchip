...
// fky_debug : add
extern  void  set_usb_current_500ma(void) ;
...
static void axp_mfd_irq_work(struct work_struct *work)
{
	...
	printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : start.\r\n") ;
	while (1) {
		if (chip->ops->read_irqs(chip, &irqs)){
			printk("read irq fail\n");
			break;
		}
//		printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : irqs=%X, chip->irqs_enabled=%X\r\n", irqs, chip->irqs_enabled) ;
		irqs &= chip->irqs_enabled;
//		printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : irqs=%X\r\n", irqs) ;
		if (irqs == 0){
			break;
		}
        ...
        // fky_debug : add
        // USB 2?¨¨?
        if (irqs & AXP_IRQ_USBIN)
        {
            set_usb_current_500ma() ;
            printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : AXP_IRQ_USBIN, i_vbus < 0.5A.\r\n") ;
        }
        // USB ¡ã?3?
        if (irqs & AXP_IRQ_USBRE)
        {
            set_usb_current_500ma() ;
            printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : AXP_IRQ_USBRE, i_vbus < 0.5A.\r\n") ;
        }

        // ¦Ì¡À?¨²1??¨²¦Ì?¡Á¡ä¨¬???¡ê?3?¦Ì?1y3¨¬?D¡ê??¨¬2a¦Ì?¨®DUSB?¨°AC¡ã?3?¨º¡À¡ê??¨ª¨ª¨º¨¨?1?¡À?¦Ì??¡ä
        if(    (1 == charger_in_logo) 
            && (   (irqs & AXP_IRQ_USBRE)
                || (irqs & AXP22_IRQ_ACRE)
               )
        )
        {
            printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : The machine will shutdown completely if USB or AC unplugged while the machine in charging.\r\n") ;
			axp_power_off() ;
	...
	printk("fky_debug : axp-mfd.c : axp_mfd_irq_work() : end.\r\n") ;
}
					...
					struct axp_platform_data *pdata)
{
	...
	ret = platform_device_add(pdev);
	printk("power supply platform_device add %d\n",i);
	if (ret)
		goto failed;
	...
	ret = platform_device_add(pdev);
	printk("gpio platform_device add %d\n",i);
	if (ret)
		goto failed;
	}
    ...
    if(1 == POWER_START)
    {
		...
		if(val & 0xF0){
	    	...
	    	if(val & 0x20){
            	...
            	mdelay(20);
		    	printk("[axp] reboot!\n");
				printk("fky_debug : display charger logo\n");
//		    	arch_reset(0,NULL);
				arm_pm_restart(0, "charge");  // fky_debug : add
